...
from cvat.apps.engine.models import (
    Catfolder, Catlog, Job, StatusChoice, Task, Project, Issue, Data,
    Comment, StorageMethodChoice, StorageChoice, Image,
    CredentialsTypeChoice, CloudProviderChoice,Sign
)
from cvat.apps.engine.models import CloudStorage as CloudStorageModel
from cvat.apps.engine.serializers import (
    AboutSerializer, AnnotationFileSerializer, BasicUserSerializer,SignSerializer,
    DataMetaSerializer, DataSerializer, ExceptionSerializer,CatSerializer,
    FileInfoSerializer, JobReadSerializer, JobWriteSerializer, LabeledDataSerializer,CatfolderSerializer,
...
from ...settings.base import MEDIA_FILES_ROOT
...
class CatlogViewSet(viewsets.ModelViewSet):
    queryset = Catfolder.objects.filter()
    ...
    def retrieve(self, request, pk):
        ...
        serializer_class = CatfolderSerializer(self.queryset.filter(projectid=pk),many=True)
        l=[]
    ...
    def data(self,request,pk):
        ...
        # queryset = Catlog.objects.filter(projectid=pk).filter(folder_name)
        
        queryset = Catlog.objects.filter(projectid=pk,folder_name=folder_name).order_by("order")
        # queryset = Catlog.objects.filter(projectid=pk,folder_name=folder_name).order_by("order")
        ...
        for i in li:
            for j,k in i.items():
                for l,m in k.items():
                    if l == "imagepath":
                        try:
                            i[j][l] = img_to_base64(m.replace("\\","/"))
                        except Exception as e:
                            pass
                    if l == "description":
                        try:
                            i[j][l] = str(i[j][l]).replace("\\n","").replace("b'","")
                        except Exception as e:
                            pass
        return Response (li)
...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def data(self, request, pk):
        ...
        if request.method == 'POST':
            ...
            for filename in os.listdir(input_path):
                if filename.endswith(".h5"):
                    ...
                    with h5py.File(filename, 'r') as f:  # open file
                        f.visit(keys.append)
                    keys = keys[3:]
                    # keys[:] = (value for value in keys if value[-6:] == "/image" or value[-12:] == "/description" )
                    util_func = lambda x: x.rsplit('/', 1)[0]
                    temp = sorted(keys, key=util_func)
                    ...
                    for list_name in res:
                        ...
                        for val in list_name:
                            list_filenames = val.split('/')
                            list_filenames[1] = list_filenames[1].rstrip()
                            if list_filenames[1] != "UNASSIGNED":
                                if (val[-12:]) == "/description":
                                    description = hdf[val][()]
                                if (val[-12:]) == "/group_image":
                                    array = hdf[val][:]
                                    try:
                                        img = PIL.Image.open(io.BytesIO(array))
                                        img_cv2 = cv2.cvtColor(np.asarray(img), cv2.COLOR_RGB2BGR)
                                        imS = cv2.resize(img_cv2, (30, 30))
                                        filepath = os.path.join(output_path, list_filenames[1])
                                        if not os.path.exists(filepath):
                                            os.mkdir(filepath)
                                        cv2.imwrite(os.path.join(filepath, list_filenames[2] + "_group_image.jpg"), imS)
                                    except:
                                        continue
                                if (val[-6:]) == "/image":
                                    array = hdf[val][:]
                                ...
                                if (val[-15:]) == "/label_mode_eva":
                                    label_mode_eva = hdf[val][()]
                                if (val[-6:]) == "/order":
                                    order = hdf[val][()]

                                    dict_val["signid"] = signid
                                    dict_val["signname"] = list_filenames[2]
                                    dict_val["projectid"] = projectid
                                    if len([x for x in list_name if x.endswith('group_image')]) > 0:
                                        dict_val["group_image"] = filepath + "\\" + list_filenames[
                                            2] + "group_image.jpg"
                                    else:
                                    ...
                                    dict_val["label_mode_dev"] = label_mode_dev
            ...
            v= os.path.join(_ddir, "/", lab.name)
            input_pat = _ddir+v
            output_pat = os.path.join(_ddir, 'labschema')
            # output_pat = os.path.join(output_pat,"/",lab.name)
            os.makedirs(output_pat, exist_ok=True)
            ...
            # for dict1 in label_list:
            #     for list1 in dict1["Attributes"]:
            #         for keys in list1:
            #             list1[keys]["mutable"] = True
            ...
            label_list=label_list.replace("CheckBox","checkbox")
            label_list=label_list.replace("Catalogue","catalogue")
            final_object_and_scene_label=label_list
            #label_list=ast.literal_eval(label_list)
            #label_list=json.dumps(label_list)
            label_list=data["LabelConfig"]["InformationElements"]

            for dicti in label_list:
                if "Device" in dicti.keys():
                    dicti.pop('Device')
                if "Source" in dicti.keys():
                    dicti.pop('Source')
                if "Channel" in dicti.keys():
                    dicti.pop('Channel')
            ...
            for dict1 in label_list:
                for list1 in dict1["Attributes"]:
                    for keys in list1:
                        list1[keys]["mutable"]= True
                        if "Values" not in list1[keys]:
                            list1[keys]["Values"]=["None"]
            #print(list1[keys])
            list_main=[]
            dict_main={}
            dict_main["name"]="Scene_Label"
            dict_main["color"]="#ffff00"
            dict_main.update(label_list[0])
            list_main.append(dict_main)
            list_main=json.dumps(list_main)
            list_main=str(list_main)
            list_main=list_main.replace("True","true")
            list_main=list_main.replace("category","name")
            list_main=list_main.replace("Attributes","attributes")
            list_main=list_main.replace("Values","values")
            list_main=list_main.replace("Type", "input_type")
            list_main=list_main.replace("TextEditor", "text")
            list_main=list_main.replace("ComboBox","select")
            list_main=list_main.replace(" \"values\": []"," \"values\": [\"\"]")
            list_main=list_main.replace("\"attributes\": [{","\"attributes\": [{\"name\":")
            list_main=list_main.replace("}}, {","},{ \"name\":")
            list_main=list_main.replace(": {",",")
            list_main=list_main.replace("}}]}","}]}")
            list_main=list_main.replace("CheckBox","checkbox")
            list_main=list_main.replace("Catalogue","catalogue")
            final_object_and_scene_label=final_object_and_scene_label+list_main
            final_object_and_scene_label=final_object_and_scene_label.replace("][",",")


            textfile="text.txt"
            path=os.path.join(output_pat,"text.txt")
            with open(path,"w") as f:
                f.write(final_object_and_scene_label)
            fi = open(path)
            data = json.load(fi)
            for i in data:
                lablename=i["name"]
                ...
                for i in attribute:
                    if i['input_type'] == "catalogue":
                        input_type = "text"
                        i["values"].append("catalogue")
                    else:
                        input_type = i['input_type']

                    if i['input_type'] in ['select','text','catalogue','number']:
                        list1=i["values"]
                        list1.insert(0, "None")
                    else:
                        list1=i["values"]
        ...
        if request_time is None or request_time < last_instance_update_time:
            ...
            if rq_job.is_finished:
                ...
                if format_name == "CANVAS 1.0":
                    ...
                    import os
                    import zipfile
                    dir_to_extract = os.path.dirname(file_path)
