...
(() => {
    ...
    class ServerProxy {
        ...
        constructor() {
            ...
            //to make the 1st api call in view_catalogue template
            async function getSigns(id) {
                const { backendAPI } = config;
                let response = null;
                try {
                    response = await Axios.get(`${backendAPI}/cat/${id}`,{
            ...
            async function populateSigns(id,fname) {
                ...
                let response = null;
                try {
                    response = await Axios.get(`${backendAPI}/cat/${id}/data/?folder_name=${fname}`,{
                       //response = await Axios.get(`http://localhost:8019/api/?pid=${id}`,{
                     proxy: config.proxy,
            ...
            async function getUpdate(id,sname){
                const { backendAPI } = config;
                let response = null;
                try {
                    response = await Axios.get(`${backendAPI}/cat/${id}/updata/?sname=${sname}`,{
                       //response = await Axios.get(`http://localhost:8019/api/?pid=${id}`,{
                     proxy: config.proxy,
            ...
            async function uploadFilesProject(formData, id) {
                const { backendAPI } = config;

                try {
                    const response = await Axios.post(`${backendAPI}/projects/${id}/data/`, formData, {
                        proxy: config.proxy,
                    });
                ...
                Object.freeze({
                    ...
                    projects: {
                        value: Object.freeze({
                            ...
                            uploadFiles: uploadFilesProject,
